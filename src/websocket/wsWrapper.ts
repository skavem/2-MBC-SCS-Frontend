...
import {
  setSearchedSong,
  setSongs,
} from "../store/actions/SongsPage/songsActions";
...
interface IWSWrapper {
  ...
  showCouplet: (couplet: ICouplet) => void;
  hideCouplet: () => void;
...
export class WSWrapper implements IWSWrapper {
  ...
  onmessage = (message: MessageEvent) => {
    ...
    if (parcel.type === pTypeEnum.show) {
      this.showHandlers[parcel.object as string as ShowHandlersEnum](
        parcel.data
      );
    }

    if (parcel.type === pTypeEnum.hide) {
      this.hideHandlers[parcel.object as string as HideHandlersEnum](
        parcel.data
      );
    }

    if (parcel.type === pTypeEnum.search) {
      this.searchHandlers[parcel.object as string as SearchHandlersEnum](
        parcel.data
      );
    }

    if (parcel.type === pTypeEnum.answer) {
      this.answerHandlers[parcel.object as string as AnsHandlersEnum](
        parcel.data
      );
    }
  };
  ...
  onopen = () => {
    ...
    let authObject = {
      type: pTypeEnum.transmitter,
      object: pObjEnum.auth,
      data: "",
    };
  ...
  establishConnection = (
    ...
    this._socket = new WebSocket(
      `ws://${ip ? ip : this._ip}:${port ? port : this._port}`
    );
    console.log("establishing connection");
    this._socket.onclose = this.onclose;
    this._socket.onopen = this.onopen;
